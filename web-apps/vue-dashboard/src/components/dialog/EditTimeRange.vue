...
export default {
  ...
  methods: {
    ...
    async submit () {
      try {
        ...
        await this.$store.dispatch('Boards/saveTimeRange', { from, to })
        this.$store.dispatch('Boards/loadObservations', { view: this.view })
  ...
  mounted () {
    this.from = this.BoardsDayFrom.format('YYYY-MM-DD')
    this.timeFrom = this.BoardsDayFrom.format('HH:mm')
    this.to = this.BoardsDayTo.format('YYYY-MM-DD')
    this.timeTo = this.BoardsDayTo.format('HH:mm')
  }
}
</script>
